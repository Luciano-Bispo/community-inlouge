...
namespace api_comil.Repositorios
{
    public class ComunidadeRepositorio : IComunidade
    {
        ...
        {
            List<Comunidade> ListaDeComunidade = await db.Comunidade.Where(w => w.DeletedoEm == null).ToListAsync();
            return ListaDeComunidade;
        }

        public async Task<Comunidade> Get(int id)
        {
            return await db.Comunidade.Where(w => w.DeletedoEm == null).SingleOrDefaultAsync(a => a.ComunidadeId == id);
        }
